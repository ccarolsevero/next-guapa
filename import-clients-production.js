...
// URL da API de produção (URL correta do deploy)
const PRODUCTION_API_URL =
  'https://nextjs-guapa-ar12ywnnv-ana-carolina-severos-projects.vercel.app/api/clients';
...
function padronizarTelefone(telefone) {
  ...
  if (telefoneLimpo.length === 11) {
    return `(${telefoneLimpo.substring(0, 2)}) ${telefoneLimpo.substring(
      2,
      7,
...
async function importarCliente(cliente) {
  try {
    const dadosCliente = {
      name: cliente.nome, // Campo correto do JSON
      email: cliente.email || gerarEmail(cliente.nome),
      phone: padronizarTelefone(cliente.telefone), // Campo correto do JSON
      birthDate: converterData(cliente.dataNascimento), // Campo correto do JSON
      address: cliente.endereco || null, // Deixar vazio se não informado
      notes:
        cliente.observacoes ||
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `HTTP ${response.status}: ${errorData.error || 'Erro desconhecido'}`,
      );
    }

    const resultado = await response.json();
    console.log(
      `✅ ${dadosCliente.name} importado com sucesso (ID: ${resultado._id})`,
    );
...
async function importarClientes() {
  try {
    ...
    console.log('Primeiro cliente:', JSON.stringify(dadosClientes[0], null, 2));
    console.log('');
    ...
    // Importar em lotes de 5 para não sobrecarregar a API
    const tamanhoLote = 5;
    for (let i = 0; i < dadosClientes.length; i += tamanhoLote) {
      const lote = dadosClientes.slice(i, i + tamanhoLote);
      console.log(
        `📦 Processando lote ${Math.floor(i / tamanhoLote) + 1}/${Math.ceil(
          dadosClientes.length / tamanhoLote,
        )}`,
      );
    ...
    fs.writeFileSync(
      'resultado-importacao-producao.json',

...
function removerDuplicados(clientes) {
  const unicos = new Map();
  const duplicados = [];
  
  clientes.forEach((cliente, index) => {
    const chave = `${cliente.nome.toLowerCase()}-${padronizarTelefone(cliente.telefone)}`;
    
    if (unicos.has(chave)) {
      duplicados.push({
        cliente: cliente.nome,
        motivo: 'Nome e telefone duplicados',
        index: index + 1
      });
    } else {
      unicos.set(chave, cliente);
    }
  });
  
  return {
    clientesUnicos: Array.from(unicos.values()),
...
async function importarCliente(cliente) {
  try {
    const dadosCliente = {
      ...
      birthDate: converterData(cliente.dataNascimento),
      address: null, // Endereço vazio por padrão
      notes: cliente.observacoes || `Cliente cadastrado em ${cliente.dataNascimento || 'data não informada'}`,
...
async function importarTodosClientes() {
  try {
    console.log('🚀 Iniciando importação de TODOS os 2000 clientes...');
    ...
    const { clientesUnicos, duplicados } = removerDuplicados(dadosClientes);
    
    console.log(`✅ Clientes únicos: ${clientesUnicos.length}`);
    console.log(`❌ Duplicados removidos: ${duplicados.length}`);
    
    if (duplicados.length > 0) {
      console.log('\n📋 DUPLICADOS REMOVIDOS:');
      duplicados.slice(0, 10).forEach(dup => {
        console.log(`  - ${dup.cliente} (linha ${dup.index})`);
      });
      if (duplicados.length > 10) {
        console.log(`  ... e mais ${duplicados.length - 10} duplicados`);
      }
    }
    ...
    const clientesNovos = clientesUnicos.filter(novoCliente => {
      const telefoneNovo = padronizarTelefone(novoCliente.telefone);
      return !clientesExistentes.some(existente => 
        existente.name.toLowerCase() === novoCliente.nome.toLowerCase() ||
        existente.phone === telefoneNovo
      );
    });
    ...
    // Importar em lotes de 20 para não sobrecarregar
    ...
    const resultadoFinal = {
      ...
      resumo: {
        ...
        duplicadosRemovidos: duplicados.length,
    ...
    fs.writeFileSync('resultado-importacao-2000-clientes.json', JSON.stringify(resultadoFinal, null, 2));
    console.log('💾 Resultados salvos em: resultado-importacao-2000-clientes.json');
